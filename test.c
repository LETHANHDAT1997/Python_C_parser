...
//utfyjdydtytydtutu
int a;void stop_webserver(void)
{
...
void switch_set_callback(void *cb)
{
    if(cb)
        p_switch_handle = cb;
        slider_set_callback(*cb);
}

jfjfghjfghjf 
slider_set_callback(int *cb    , char *c)
{
    if(cb)
        p_slider_handle = cb;
}
...
void rgb_set_callback(void *cb)
{
    if(cb)
        p_rgb_handle = cb;
};int a; void rgb_set_callback(void *cb)
{
    if(cb)
        p_rgb_handle = cb;
}
  ...
  ******************************************************************************
  ...
  (+) 12-bit resolution
    ...
    [..]
     ...
     By default, after the jfjfghjfghjf slider_set_callback(void *cb) and when the state is HAL_ADC_STATE_RESET
