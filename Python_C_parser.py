...
def extract_function_details(code):
    # Define the regex pattern with optional space before the parenthesis
    pattern = re.compile(r'(\w[\w\s\[\],<>]*?)\s+(\w+)\s*\((.*?)\)\s*')
    ...
    for match in matches:
        ...
        # Clean up the return type and params
        return_type = return_type.strip()
        params = params.strip()
        function_details.append((return_type, function_name, [param.strip() for param in params.split(',')]))

    return function_details

output_code = open_file()
# print(output_code)
function_details = extract_function_details(output_code)
for detail in function_details:
    print(f"Return type: {detail[0]}")
    print(f"Function name: {detail[1]}")
    print(f"Parameters: {detail[2]}")
    print()
